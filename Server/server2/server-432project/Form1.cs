...
namespace server_432project
{
    ...
    public partial class Form1 : Form
    {
        ...
        string RSAPrivateKey3072_Server2;
        ...
        private void button1_Click(object sender, EventArgs e) //connect button
        {
            ...
            if (Int32.TryParse(port_to_be_connected.Text, out port))
            {
                try
                {
                    ...
                    // Send ID as message
                    string username = "Server 2";



                    if (server1conn == true)
                    {
                        string mes = "Server 2-serversconnected";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string IP = "localhost";
            int port;
            if (Int32.TryParse(port_server1.Text, out port))
            {
                try
                {
                    ...
                    richTextBox1.AppendText("Connecting to server 1\n");
                    // Send ID as message
                    string username = "Server 2";
            ...
            while (masterConnected)
            {
                try
                {
                    ...
                    else if (message.Length > 21 && message.Substring(0, 21) == "verificationSignature")
                    {
                        ...
                        Task verifySignatureThread = new Task(() => verifySignature("MASTER", message, fileContent));
        ...
        private void Receive_from_server(string user)
        {
            while (server1conn)
            {
                try
                {
                    ...
                    {
                        richTextBox1.AppendText("Server 1 sent me a file \n");
                        Thread receiverep2Thread = new Thread(() => ReceivefromServer(server1, "Server 1", message)); // updated
                        receiverep2Thread.Start();
                    }
                    else if (message.Length > 21 && message.Substring(0, 21) == "verificationSignature")
                    {
                        richTextBox1.AppendText("Server 1 sent me its signature \n");
                        Thread verifySignatureThread = new Thread(() => verifySignature("Server 1", message, fileContent));
                        verifySignatureThread.Start();
                    }
                    else if (message.Length > 9 && message.Substring(0, 9) == "HMACError")
                    {
                        richTextBox1.AppendText("Master sent me HMAC error message... Now verifying signature... \n");
                        verifyHMACError(server1, message, "Server 1");
        ...
        async void verifySignature(string server, string message, string fileContent)
        {
            ...
            if (message.IndexOf("P") + 1 != message.Length && verificationResult)
            {
                ...
                if (senttoserver1.Contains(filename) && senttomaster.Contains(filename))
                {
            ...
            if (verificationResult == true)
            {
                ...
                Byte[] buffer4 = Encoding.Default.GetBytes("Server2 verified my signature.");
                if (server == "Server 1")
                {
        ...
        {
            if (server1conn && masterConnected && filenamesqueue.Any())
            {
                foreach (string filename in filenamesqueue.ToList())
                {
                    string filepath = @"c:\Server2\Files\";
        ...
        private void verifyKey(Socket master, string message, string user)
        {
            ...
            {
                RSAPrivateKey3072_Server2 = fileReader.ReadLine();
            }
            ...
            byte[] session_key = decryptWithRSA(enc_session_key, 3072, RSAPrivateKey3072_Server2);
        ...
        private void Accept()
        {
            while (listening)
            {
                try
                {
                    ...
                    //Byte[] buffer2 = Encoding.Default.GetBytes("You have connected to the SERVER2");
        ...
        private void Check(Socket thisClient) // updated
        {
            ...
            while (trying && !terminating)
            {
                try
                {
                    ...
                    else if (server_id == "Server 1")
                    {
                        server1 = thisClient;
                        richTextBox1.AppendText("Can receive from and send to Server 1 \n");
                        server1conn = true;
        ...
        private void ReceivefromServer(Socket server1, string serverid, string message)
        {
            richTextBox1.AppendText("Server1 is sending me a file \n");
            verifyReplicate(server1, message);
        ...
        void verifyReplicate(Socket s, string message)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    byte[] signature = signWithRSA(errorMessage, 3072, RSAPrivateKey3072_Server2);
            ...
            catch
            {
                ...
                Byte[] buffer9 = Encoding.Default.GetBytes("Server 2 failed decryption." + dec_filename);
                s.Send(buffer9);
                check = false;
            }
            ...
            if (check)
            {
                ...
                byte[] signature = signWithRSA(dec_file, 3072, RSAPrivateKey3072_Server2);
        ...
        public async void SendToServers(string file, string filename)
        {
            ...
            if (masterConnected && server1conn && filenamesqueue.Contains(filename))
            {
            ...
            else
            {
                ...
                //richTextBox1.AppendText("server1conn:" + server1conn + " masterconn:" + masterConnected);
        ...
        void Download_Function(Socket s, string message)
        {
            ...
            if (System.IO.File.Exists(download_path))
            {
                ...
                byte[] signature = signWithRSA(download_fileContent, 3072, RSAPrivateKey3072_Server2);
            ...
            else
            {
                ...
                byte[] signature = signWithRSA(errormes, 3072, RSAPrivateKey3072_Server2);
        ...
        void verifyFile(Socket s, string message)
        {
            ...
            try
            {
                byte[] decrypted_AES_Key = decryptWithRSA(enc_AES_Key, 3072, RSAPrivateKey3072_Server2);
                byte[] decrypted_iv = decryptWithRSA(enc_iv_Key, 3072, RSAPrivateKey3072_Server2);
            ...
            if (check)
            {
                ...
                //signature, server signs file content with priv key
                //client verifs with public key
                byte[] signature = signWithRSA(string_dec_file, 3072, RSAPrivateKey3072_Server2);
        ...
        private void port_to_be_connected_TextChanged(object sender, EventArgs e)
        {
